...
- 桌面轮询拆分为两秒实时状态与十秒配置目录刷新，避免每轮重复请求所有资源。
- 窗口隐藏到托盘或页面不可见时暂停自动轮询，重新可见后立即恢复实时状态。
- 自动刷新不再清除用户正在查看的操作错误，并防止同类刷新请求重叠执行。

- Updater 安装前重新校验用户确认的目标版本，避免检查与安装之间 Release 变化导致版本不一致。
- 软件更新新增下载百分比/已下载大小反馈，并使用独立错误状态避免被两秒状态轮询清除。
- Profile 删除新增名称明确的二次确认，降低误删当前场景的风险。
